...
- 优化 SaaS 员工 Agent 使用体验：执行期间仅自动展开当前工具或子智能体任务，结束后默认收起全部轨迹；资料摘要改为部门与员工姓名，设置中隐藏本地角色及基本设置、OCR 和沙盒环境变量；退出时先清理本地状态再通知后端并稳定跳转登录页；NewAPI 模型同步提交后立即刷新共享缓存；Agent 端展示员工有权读取的租户知识库和目录，隐藏知识库结构管理入口，仅 `manage_scope` 命中的员工可管理文档。

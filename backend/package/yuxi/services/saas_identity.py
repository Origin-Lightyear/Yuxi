...
SaaS 登录时把 Tenant 侧的 tenant_id / employee_id 持久化到 user_config，
...
async def save_saas_employee_context(db: AsyncSession, uid: str, tenant_id: int, employee_id: int) -> None:
    """登录成功后持久化员工身份（upsert user_config）。"""
    now = utc_now_naive()
    result = await db.execute(
        update(UserConfig)
        .where(UserConfig.uid == uid)
        .values(tenant_id=tenant_id, employee_id=employee_id, updated_at=now)
    )
...
async def get_saas_employee_context(db: AsyncSession, uid: str) -> SaasEmployeeContext | None:
    ...
    if record is None or record.tenant_id is None or record.employee_id is None:
        return None
    return SaasEmployeeContext(uid=uid, tenant_id=record.tenant_id, employee_id=record.employee_id)

...
export const saasApi = {
  /**
   * 查询 SaaS 模式状态（公开接口）
   * @returns {Promise<{saas_enabled: boolean, tenant_id: number|null}>}
   */
  getStatus: () => apiGet('/api/system/saas-status', {}, false),

  /**
   * 获取租户配置（含 MCP URL，需登录）
   * @returns {Promise} - 租户配置
   */
  getTenantConfig: () => apiGet('/api/system/tenant-config')
}
